...
# Explode database
    ...
    df = pd.DataFrame(guts, columns =['code', 'name', 'location', 'reference product', 'categories', 'classifications', 'exchanges'])
    ...
    html_path = os.path.join(tmp, db.name +"_0:5000.html")
    df.to_html(html_path, max_rows=1000)
